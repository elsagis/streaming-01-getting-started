...
Try this to activate your base conda environment. 

1. Close VS Code 
1. On Windows, open Anaconda command prompt - on Mac, open Terminal. 
1. Run `conda init`.  That is, type conda space init and hit the Enter or Return key. 
1. Now, open your repo folder in VS Code.
1. Run the code in "about.py" using the > key or other option listed in [Run Hello World](https://code.visualstudio.com/docs/python/python-tutorial#_run-hello-world).
1. Does your active conda env show `base`? If so that's excellent!  If not, and everything runs, just continue for now. More on environments as we go. 

The about.txt file generated by your about module provides useful information about your Python environment. 
Review these paths carefully - it can help locate and better understand your Python installation / environment. 
